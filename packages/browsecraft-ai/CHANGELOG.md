...
- [`bca605a`](https://github.com/rik9564/browsecraft/commit/bca605a0058c5ce98ede75c28de2a25788073878) Thanks [@rik9564](https://github.com/rik9564)! - Fix a crash ("Dynamic require of zlib is not supported") in `compareScreenshots()` when loaded via the ESM build. A stray CJS `require('node:zlib')` inside the visual-diff module broke any second call to `expect(page).toMatchSnapshot()` that needed to diff against an existing baseline.
