...
// Optimization: Pre-compiled regex avoids callback allocation and array iteration overhead
// Expected impact: ~3x faster execution for sensitive key matching compared to Array.some
...
export function sanitize(obj: unknown): unknown {
	...
	for (const [key, value] of Object.entries(record)) {
		...
		if (SENSITIVE_PATTERN.test(lowerKey)) {
		...
		if (
			...
			SENSITIVE_PATTERN.test((record.name as string).toLowerCase())
		) {
