...
// Optimization: Pre-compiled regex replaces Array.some for faster string matching
// Expected impact: ~3x faster key matching in recursive sanitize function
...
export function sanitize(obj: unknown): unknown {
	...
	for (const [key, value] of Object.entries(record)) {
		...
		if (
			...
			SENSITIVE_KEY_PATTERN.test(record.name as string)
		) {
