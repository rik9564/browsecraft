...
// Optimization: Pre-compile sensitive key matching into a single non-global Regular Expression instead of array iteration.
// Expected impact: Significantly reduces object allocation and loop iteration overhead during high-frequency network event sanitization.
...
export function sanitize(obj: unknown): unknown {
	...
	for (const [key, value] of Object.entries(record)) {
		...
		if (SENSITIVE_REGEX.test(lowerKey)) {
		...
		if (
			...
			SENSITIVE_REGEX.test(record.name as string)
		) {
