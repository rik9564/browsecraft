...
// Performance Optimization: For high-frequency string matching (e.g., sanitization of fast-firing socket payloads),
// `RegExp.test` with a pre-compiled regex is preferred over `Array.some(str.includes)` as it avoids
// object allocation, redundant traversal, and is significantly faster in Node.js.
...
export function sanitize(obj: unknown): unknown {
	...
	for (const [key, value] of Object.entries(record)) {
		...
		if (
			...
			SENSITIVE_KEYS_REGEX.test(record.name)
		) {
