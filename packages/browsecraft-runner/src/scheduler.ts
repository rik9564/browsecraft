...
export class Scheduler {
	...
	private async runParallel(
		...
		for (const browser of browsers) {
			...
			// Calculated counts and duration in a single pass to avoid N+1 array iterations (multiple filter/reduce calls).
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			let duration = 0;
	...
	private async runSequential(
		...
		for (const browser of browsers) {
			...
			// ⚡ Bolt: Performance Optimization
			// Calculated counts in a single pass to avoid N+1 array iterations.
			let passed = 0;
			let failed = 0;
			let skipped = 0;
	...
	private buildResult(
		...
		// ⚡ Bolt: Performance Optimization
		// Calculated all browser-specific and global counts in a single O(N) loop
		// to avoid redundant O(N) array filter/reduce operations across N browsers.
		let totalPassed = 0;
		let totalFailed = 0;
		let totalSkipped = 0;
		...
		>();
		for (const browser of browsers) {
			browserStats.set(browser, { passed: 0, failed: 0, skipped: 0, duration: 0, results: [] });
		}
		...
		for (const r of allResults) {
			...
			const bStats = browserStats.get(r.worker.browser);
			if (bStats) {
				bStats.results.push(r);
				if (r.status === 'passed') bStats.passed++;
				else if (r.status === 'failed') bStats.failed++;
				else if (r.status === 'skipped') bStats.skipped++;
				bStats.duration += r.duration;
			}
		}
