...
export class Scheduler {
	...
	private async runParallel(
		...
		for (const browser of browsers) {
			// Bolt Optimization: Calculate metrics in a single O(N) pass
			// instead of O(N*4) with intermediate array allocations via .filter().length
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			let duration = 0;
	...
	private async runSequential(
		...
		for (const browser of browsers) {
			...
			// Bolt Optimization: Single O(N) pass for status counts
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			for (const r of results) {
	...
	private buildResult(
		...
		// Bolt Optimization: Replaced O(N*3) total passed/failed/skipped counting with single O(N) loop
		let totalPassed = 0;
		let totalFailed = 0;
		let totalSkipped = 0;
