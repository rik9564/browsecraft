...
export class Scheduler {
	...
	private async runParallel(
		...
		for (const browser of browsers) {
			// Optimization: Calculate stats in a single pass to avoid N+1 traversals
			// and intermediate allocations from repeated .filter() and .reduce() calls
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			let duration = 0;
	...
	private async runSequential(
		...
		for (const browser of browsers) {
			...
			// Optimization: Calculate stats in a single pass to avoid N+1 traversals
			// and intermediate allocations from repeated .filter() calls
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			for (const r of results) {
	...
	private buildResult(
		...
		// Optimization: Use a single O(N) loop and a hash map (O(1) lookups) to aggregate
		// overall totals and per-browser results simultaneously. This avoids redundant
		// array traversals and intermediate allocations caused by repeated .filter() and .reduce() calls.
		let totalPassed = 0;
		let totalFailed = 0;
		let totalSkipped = 0;
		...
		for (const r of allResults) {
			...
			const stats = browserStats.get(r.worker.browser);
			if (stats) {
