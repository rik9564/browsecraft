...
export class Scheduler {
	...
	private async runParallel(
		...
		for (const browser of browsers) {
			...
			// Performance Optimization: Compute counts and duration in a single loop
			// to avoid multiple O(N) array traversals via .filter() and .reduce().
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			let duration = 0;
	...
	private buildResult(
		...
		// Performance Optimization: Compute total counts in a single loop
		// to avoid multiple O(N) array traversals via .filter().
		let totalPassed = 0;
		let totalFailed = 0;
		let totalSkipped = 0;
