...
export class Scheduler {
	...
	private async runParallel(
		...
		for (const browser of browsers) {
			...
			// Optimization: Compute multiple counts and sums in a single O(N) pass
			// instead of multiple O(N) filter/reduce calls to avoid array allocations
			let passed = 0;
			let failed = 0;
			let skipped = 0;
			let duration = 0;
	...
	private async runSequential(
		...
		for (const browser of browsers) {
			...
			// Optimization: Compute multiple counts in a single O(N) pass
			// instead of multiple O(N) filter.length calls to avoid array allocations
			let passed = 0;
			let failed = 0;
			let skipped = 0;
